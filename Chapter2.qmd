...
Round 1: black Round 2: white Total ww =\> 2 ww =\> 0 bw =\> 1 0 bb =\> 0
...
p(t\|A) = 0.1 p(t\|B) = 0.2 p(A) = p(B) = 0.5

```{r 2H1}
# get the posterior for the first twins
p_A <- 0.5
p_B <- 0.5
...
p_A_new_prior <- p_A_given_t1


p_B_given_t1 <-  p_t_given_B * p_B / (p_A*p_t_given_A + p_B * p_t_given_B)
p_B_new_prior <- p_B_given_t1

# probability of getting twins againg = likelyhood  of seeing twins given A  + seeing twins given B
...
result <- p_t2_given_A + p_t2_given_B
...
p_A_given_testA <- (p_testA_given_A * p_A ) / (p_testA_given_A * p_A  + # correct test result
...
p_A_given_testA_2 <- (p_testA_given_A * p_A_newest_prior ) / (p_testA_given_A * p_A_newest_prior  + # correct test result
